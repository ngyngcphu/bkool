...
class StaticChecker(BaseVisitor,Utils):
    ...
    def visitFuncDecl(self, ctx:FuncDecl, o:object):
        ...
        env = reduce(lambda acc, cur: self.visit(cur, acc), ctx.param, [])
        reduce(lambda acc, cur: self.visit(cur, acc), ctx.body, env)
...
if __name__ == '__main__':
    ast_tree = Program([VarDecl("a",IntType()),ConstDecl("b",IntLit(3)),FuncDecl("a",[],[])])
